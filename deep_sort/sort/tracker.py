...
class Tracker:
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            #match with previous video
            # if self.Flag:
            #     file = os.getcwd() + '\\1.csv' 
            #     feature_data = pd.DataFrame(features)
            #     feature_data.to_csv(file, index=True, mode='a+', header=False)
            #     print("save features")
            # else:
            #     file = os.getcwd() + '\\1.csv'
            #     prev_feature  = pd.read_csv(file)
            #     prev_feature = prev_feature.values[-1][1:].reshape(1,512)
            #     #label = prev_feature.values[-1][0]

            #     #是一个对应所有目标和特征的矩阵，因为只有一个目标所以是一个数值，其实应该不是的
            #     cost_matrix = self.metric.distance(prev_feature, targets)
            #     print(cost_matrix)
            #     if cost_matrix < self.metric.matching_threshold: #(x)
            #         print("same car")
            #         self.Flag = False
            #     else:
            #         print("not the same car")
            #         count = self.count + 1
            #         if count > 10:
            #             self.Flag = True
